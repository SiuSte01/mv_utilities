...
if(! defined $params{Codebase})
{
	print "must specify Codebase value\n";
	exit;
}
elsif(defined $params{Codebase})
{
	if($params{Codebase} ne "Dev" && $params{Codebase} ne "Prod_NewWH")
	{
		print "Codebase must be either Dev or Prod_NewWH\n";
		exit;
	}
}
...
my $migdir="/vol/datadev/Statistics/Projects/HGWorkFlow/$params{Codebase}/AdvisoryBoard";
...
{	
	...
	my $memoryHash;
	my $hgRoot = "/vol/datadev/Statistics/Projects/HGWorkFlow";
	...
	foreach my $x (@reqFiles)
	{
		unless(-e $fxFiles . "/" . $x)
		{
			system("cp " . $hgRoot . "/InputDataFiles_NewWH/" . $x . " " . $fxFiles . "/" . $x);
		}
	}
	...
	#createxwalks.py is fast enough that it does not warrant an email
	#system("perl /vol/datadev/Statistics/Projects/HGWorkFlow/$params{Codebase}/sendemail.pl createxwalks");
	...
	SETTLOOP: foreach my $sett (sort keys %settings)
	{
		...
		my $mbTimeBegin = time();
		system("perl /vol/datadev/Statistics/Projects/HGWorkFlow/$params{Codebase}/multiBucket_ABCPM.pl $params{Client} $sett $params{Codebase}");
