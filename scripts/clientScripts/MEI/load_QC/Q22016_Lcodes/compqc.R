...
last2sum<-data$Q32014+data$Q42014
prior2sum<-data$Q12014+data$Q22014
...
pdf("compqc_lcodes_Q3Q42014.pdf")
...
write.table(data,file="data_lcodes_Q3Q42014.txt",col.names=T,row.names=F,quote=F,
sep="\t",na="")


diff<-log10((data$Q22014+data$Q12014)/(data$Q32014+data$Q42014))
diff2<-abs((data$Q22014+data$Q12014)-(data$Q32014+data$Q42014))
i1<-which(diff2<= 10)
...
write.table(bad.records,file="lcodes_baddata.txt",col.names=T,row.names=F,quote=F,
sep="\t",na="")
