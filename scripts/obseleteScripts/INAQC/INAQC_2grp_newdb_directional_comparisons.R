...
olddir<-"/vol/cs/clientprojects/rdh_ina_test/2016_11_11_ina_dxcohort_new_method_no_direction/CKD3_DxCohort_PIIDTOPIID/Comb"
newdir<-"/vol/cs/clientprojects/rdh_ina_test/2016_08_31_ina_directionality_dxcohort4/CKD3_DxCohort_PIIDTOPIID/Comb"

outdir<-"/vol/cs/CS_PayerProvider/Ryan/R/INAQC_test/direct_test_new"
...
split.links<-as.data.frame(t(apply(do.call(rbind,strsplit(new.links$VAR1,split=":")),FUN=sort,MARGIN=1)))
new.links$ordered_VAR1<-paste(split.links[,2],split.links[,1],sep=":")
co.links<-merge(old.links,new.links[,c(2:4)],by.x="VAR1",by.y="ordered_VAR1",all=T)
...
### links
#split.links<-as.data.frame(t(apply(do.call(rbind,strsplit(new.links$VAR1,split=":")),FUN=sort,MARGIN=1)))
#new.link.id<-paste(split.links[,2],split.links[,1],sep=":")
###START HERE - old.linksVAR1 is in REVERSE alphabetical. Already switched above, but needs testing. Run from: /vol/cs/clientprojects/rdh_ina_test
#links.loss.count<-length(old.links$VAR1[which(! old.links$VAR1 %in% new.link.id)])
#links.gain.count<-length(new.link.id[which(! new.link.id %in% old.links$VAR1)])
...
#links.l.count <- paste("count of dropped links = ",links.loss.count)
#links.g.count <- paste("count of net new links = ",links.gain.count)
#links.l.pct1<- (links.loss.count/old.link.rows)*100
#links.g.pct1<- (links.gain.count/old.link.rows)*100
...
if(!grp2test){
 ...
 denom.table1<-(table(co.denom[,2],co.denom[,6],exclude=NULL))
 denom.table2<-(table(co.denom[,3],co.denom[,7],exclude=NULL))
}
