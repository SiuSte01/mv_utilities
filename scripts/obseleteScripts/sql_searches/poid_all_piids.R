#needs edited. Idea is to return for a POID all PIID volume in 1500 and UB claims.
...
###find claims by piid, report counts by poid.
 query<-paste("select count (distinct prof.claim_id), fxwalk.id_value,xwalk.id_value from CLAIMSWH.PROF_CLAIMS prof inner join CLAIMSWH.PRACTITIONER_GROUP_MEMBERS pgm
 ...
 where fxwalk.id_value in ('",paste(args,collapse="','"),"') and xwalk.id_type = 'PIID' and fxwalk.id_type = 'POID'
 and prof.claim_through_date between vend.first_vend_date and vend.last_vend_date
 and prof.load_batch <= vend.last_vend_batch
 group by fxwalk.id_value, xwalk.id_value order by fxwalk.id_value, count (distinct prof.claim_id) desc",sep="")
 ...
 where fxwalk.id_type = 'POID' and fxwalk.id_value in ('",paste(args,collapse="','"),"') and xwalk.id_type = 'PIID'
 and inst.claim_through_date between vend.first_vend_date and vend.last_vend_date
 and inst.load_batch <= vend.last_vend_batch
 group by fxwalk.id_value, xwalk.id_value order by fxwalk.id_value, count (distinct inst.claim_id) desc",sep="")
