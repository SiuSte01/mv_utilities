...
###find claims by piid, report counts by poid.
 query<-paste("select count (distinct prof.claim_id),fxwalk.id_value,vend.vendor_name,pos.description
 ...
 where fxwalk.id_value in ('",paste(args,collapse="','"),"') and fxwalk.id_type = 'POID'
 and prof.claim_through_date between vend.first_vend_date and vend.last_vend_date
 and prof.load_batch <= vend.last_vend_batch
 group by fxwalk.id_value, vend.vendor_name,pos.description order by fxwalk.id_value, count (distinct prof.claim_id) desc",sep="")
 ...
 query2<-paste("select count (distinct inst.claim_id), vend.vendor_name, fxwalk.id_value from CLAIMSWH.INST_CLAIMS inst
 inner join CLAIMSWH.PRACTITIONER_GROUP_MEMBERS pgm
 ...
 where fxwalk.id_type = 'POID' and fxwalk.id_value in ('",paste(args,collapse="','"),"')
 and inst.claim_through_date between vend.first_vend_date and vend.last_vend_date
 and inst.load_batch <= vend.last_vend_batch
 group by fxwalk.id_value, vend.vendor_name order by fxwalk.id_value, count (distinct inst.claim_id) desc",sep="")
