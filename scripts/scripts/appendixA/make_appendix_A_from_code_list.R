...
if("CODE_TYPE" %in% colnames(codes)){colnames(codes)[which(colnames(codes)=="CODE_TYPE")]<-"CODE_GROUP_TYPE"}
...
notfound<-as.data.frame(matrix(ncol=ncol(codes),nrow=0))
colnames(notfound)<-colnames(codes)
...
if ("DX" %in% codes$CODE_GROUP_TYPE){
 ...
 codes$Description[which(codes$CODE_GROUP_TYPE=="DX")]<-dx_results$DESCRIPTION[match(paste(codes$CODE[which(codes$CODE_GROUP_TYPE=="DX")],codes$CODE_SCHEME[which(codes$CODE_GROUP_TYPE=="DX")],sep="_"),paste(dx_results$ADDNL_DIAGNOSIS_CODE,dx_results$CODE_SCHEME,sep="_"))]
 ...
 codes$Out_code[which(codes$CODE_GROUP_TYPE=="DX")]<-dx_results$DIAGNOSIS_CODE[match(paste(codes$CODE[which(codes$CODE_GROUP_TYPE=="DX")],codes$CODE_SCHEME[which(codes$CODE_GROUP_TYPE=="DX")],sep="_"),paste(dx_results$ADDNL_DIAGNOSIS_CODE,dx_results$CODE_SCHEME,sep="_"))]
 notfound<-rbind(notfound,codes[which(codes$CODE_GROUP_TYPE=="DX" & ! codes$CODE %in% dx_results$ADDNL_DIAGNOSIS_CODE),])
}
...
if ("PX" %in% codes$CODE_GROUP_TYPE){
 ...
 codes$Description[which(codes$CODE_GROUP_TYPE=="PX")]<-px_results$DESCRIPTION[match(paste(codes$CODE[which(codes$CODE_GROUP_TYPE=="PX")],codes$CODE_SCHEME[which(codes$CODE_GROUP_TYPE=="PX")],sep="_"),paste(px_results$ADDNL_PROCEDURE_CODE,px_results$CODE_SCHEME,sep="_"))]
 codes$Out_code[which(codes$CODE_GROUP_TYPE=="PX")]<-px_results$PROCEDURE_CODE[match(paste(codes$CODE[which(codes$CODE_GROUP_TYPE=="PX")],codes$CODE_SCHEME[which(codes$CODE_GROUP_TYPE=="PX")],sep="_"),paste(px_results$ADDNL_PROCEDURE_CODE,px_results$CODE_SCHEME,sep="_"))]
 notfound<-rbind(notfound,codes[which(codes$CODE_GROUP_TYPE=="PX" & ! codes$CODE %in% px_results$ADDNL_PROCEDURE_CODE),])
}
...
if ("DRG" %in% codes$CODE_GROUP_TYPE){
 ...
 notfound<-rbind(notfound,codes[which(codes$CODE_GROUP_TYPE=="DRG" & ! codes$CODE %in% drg_results$DRG_CODE),])
}

codes$Out_code[which(is.na(codes$Out_code))]<-codes$CODE[which(is.na(codes$Out_code))]
notfound$Out_code[which(is.na(notfound$Out_code))]<-notfound$CODE[which(is.na(notfound$Out_code))]

output<-codes[,c("CODE_GROUP_NAME","Out_code","CODE_SCHEME","Description")]
colnames(output)<-c("Bucket","Code","Code Type","Description")

notfound<-notfound[,c("CODE_GROUP_NAME","Out_code","CODE_SCHEME","Description")]
colnames(notfound)<-c("Bucket","Code","Code Type","Description")

write.table(output,file.path(dirname(infile),"appendixA.tab"),sep="\t",quote=F,row.names=F)
if(nrow(notfound) > 0){write.table(notfound,file.path(dirname(infile),"appendixA_notfound.tab"),sep="\t",quote=F,row.names=F)}
