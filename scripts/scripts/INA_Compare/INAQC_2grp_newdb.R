...
old.denom1.rows<-nrow(subset(old.denom,(!is.na(old.denom$Grp1Rank))))
old.denom2.rows<-nrow(subset(old.denom,(!is.na(old.denom$Grp2Rank))))

new.link.rows<-nrow(new.links)
new.denom.rows<-nrow(new.denom)
new.denom1.rows<-nrow(subset(new.denom,(!is.na(new.denom$Grp1Rank))))
new.denom2.rows<-nrow(subset(new.denom,(!is.na(new.denom$Grp2Rank))))
...
### denom
denom.1.loss <- subset(co.denom, is.na(Grp1Rank.y)&!is.na(Grp1Rank.x))
denom.2.loss <- subset(co.denom, is.na(Grp2Rank.y)&!is.na(Grp2Rank.x))
denom.1.gain <- subset(co.denom, is.na(Grp1Rank.x)&!is.na(Grp1Rank.y))
denom.2.gain <- subset(co.denom, is.na(Grp2Rank.x)&!is.na(Grp2Rank.y))

denom.l1.count <- paste("count of dropped Grp1 IDs = ",nrow(denom.1.loss))
denom.l2.count <- paste("count of dropped Grp2 IDs = ",nrow(denom.2.loss))

denom.g1.count <- paste("count of net new Grp1 IDs = ",nrow(denom.1.gain))
denom.g2.count <- paste("count of net new Grp2 IDs = ",nrow(denom.2.gain))
denom.l1.pct1 <- (nrow(denom.1.loss)/old.denom1.rows)*100
denom.l2.pct1 <- (nrow(denom.1.loss)/old.denom2.rows)*100
denom.g1.pct1 <- (nrow(denom.1.gain)/old.denom1.rows)*100
denom.g2.pct1 <- (nrow(denom.2.gain)/old.denom2.rows)*100

denom.l1.pct <- paste("percent of dropped Grp1 IDs = ",denom.l1.pct1)
denom.l2.pct <-paste("percent of dropped Grp2 IDs = ",denom.l2.pct1)
denom.g1.pct <- paste("percent of net new Grp1 IDs = ",denom.g1.pct1)
denom.g2.pct <- paste("percent of net new Grp2 IDs = ",denom.g2.pct1)
...
denom.table<-(table(co.denom$Grp1Rank.x,co.denom$Grp1Rank.y))
print("11 by 11 table for denom Grp 1 ranks")
print(denom.table)

denom.table2<-(table(co.denom$Grp2Rank.x,co.denom$Grp2Rank.y))
print("11 by 11 table for denom Grp 2 ranks")
print(denom.table2)

rank.table<-(table(co.links$cq.x,co.links$cq.y))
...
">2xDecr"))
cat("Grp1 ranked denom pat count change distribution\n")
...
print.dx.10<-paste("Grp1 denom good 10% = ",dx.ranked.good.10)
...
print.dx.25<-paste("Grp1 denom good 25% = ",dx.ranked.good.25)

#calculate % good for group 2 
diff.breaks<-c(-Inf,log10(0.5),log10(1/1.1),log10(1/1.00001),
log10(1.00001),log10(1.1),log10(2),Inf)
g2.ranked.count.diff<-log10(co.ranked$Grp2Cnt.x/co.ranked$Grp2Cnt.y)
g2.ranked.binned<-cut(g2.ranked.count.diff,breaks=diff.breaks,labels=c(">2xIncr",
"Betwn10%and2xIncr","<10%Incr","NoChange","<10%Decr","Betwn10%and2xDecr",
">2xDecr"))
cat("Grp2 ranked denom pat count change distribution\n")

dbin.label <- paste(">2xIncr",
"Betwn10%and2xIncr","<10%Incr","NoChange","<10%Decr","Betwn10%and2xDecr",
">2xDecr")
g2.bin.ranked<-print(table(g2.ranked.binned),include.colnames=T)

ranked.diff<-log10(co.ranked$Grp2Cnt.x/co.ranked$Grp2Cnt.y)
ranked.diff2<-abs(co.ranked$Grp2Cnt.x-co.ranked$Grp2Cnt.y)
i1<-which(ranked.diff2 <= 5)
i2<-which((ranked.diff < log10(1.1)) & (ranked.diff > log10(1/1.1)))
i<-union(i1,i2)
print(length(i1))
...
g2.ranked.good.10<-100*(length(i)/length(ranked.diff2))
print.g2.10<-paste("Grp2 denom good 10% = ",g2.ranked.good.10)
...
g2.ranked.good.25<-100*(length(i)/length(ranked.diff2))
print.g2.25<-paste("Grp2 denom good 25% = ",g2.ranked.good.25)
...
cat("Churn results","\n", print.dx.10, "\n", print.dx.25, "\n",print.g2.10, "\n", print.g2.25, "\n", "denom grp 1 change distr", "\n", 
dbin.label, "\n", dx.bin.ranked, "\n", "denom grp 2 change distr","\n", dbin.label, "\n", g2.bin.ranked, "\n", 
"link change distr", "\n", links.good.pct, "\n", lbin.label,"\n", bin.links, "\n", links.l.count, "\n", 
links.l.pct, "\n", links.g.count, "\n", links.g.pct, "\n", denom.l1.count, "\n", denom.l1.pct, "\n", denom.l2.count, "\n", denom.l2.pct, 
"\n", denom.g1.count, "\n", denom.g1.pct,"\n",  denom.g2.count, "\n", denom.g2.pct, file="Churn_statistics.txt",sep="\n")
...
ylab="New",main="Denom G1 Patients",pch=4,cex=.5,
...
xlab="Old",ylab="New",main="denom g1 patients",xlim=c(0.1,1),ylim=c(0.1,1))
lines(c(0.1,1),c(0.1,1),lwd=3,col="red")

# group 2

ranked.minval<-min(c(co.ranked$Grp2Cnt.x,co.ranked$Grp2Cnt.y))
ranked.lminval<-log10(ranked.minval)
ranked.maxval<-max(c(co.ranked$Grp2Cnt.x,co.ranked$Grp2Cnt.y))
ranked.lmaxval<-log10(ranked.maxval)

#Plots for Ranked denom based on num of dx patients

ranked.max<-max(max(co.ranked$Grp2Cnt.x),max(co.ranked$Grp2Cnt.y))
plot(co.ranked$Grp2Cnt.x,co.ranked$Grp2Cnt.y,xlab="Old",
ylab="New",main="Denom G2 Patients",pch=4,cex=.5,
xlim=c(0.1,1000),ylim=c(0.1,1000),log="xy")
lines(c(0.1,1000),c(0.1,1000),lwd=3,col="red")
# heat plot
# denom Dx connections
Lab.palette <- colorRampPalette(c("blue", "orange", "red"), space = "Lab")
smoothScatter(log10(co.ranked$Grp2Cnt.x),log10(co.ranked$Grp2Cnt.y),colramp = Lab.palette,
xlab="Old",ylab="New",main="denom g2 patients",xlim=c(0.1,1),ylim=c(0.1,1))
lines(c(0.1,1),c(0.1,1),lwd=3,col="red")
