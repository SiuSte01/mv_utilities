###?in some versions of the INA, possibly all 1 groups, they do not bring grp1cnt into the denom_fordelivery file from ranked denom. this will need to be addressed.
...
if(! is.ina & ! is.pxdx){stop("directories should be Comb if INA or QA if PxDx")}
...
#uses already binned data to construct bar plots
...
#test if change is less than a percent or a hard coded number
test.range<-function(percent,number,df.test,input1,input2,text,na.rm=T){
 if (! any(grepl(input1,colnames(df.test))) | ! any(grepl(input2,colnames(df.test)))  ){stop("fields not found")}
 if(na.rm){df.test<-df.test[which(df.test[[input1]] != 0.1 & ! is.na(df.test[[input1]]) & df.test[[input2]] != 0.1 & ! is.na(df.test[[input2]])),]}
...
old.ranked<-old.ranked[which(old.ranked$HMS_ID %in% old.denom$HMS_ID),]
new.ranked<-new.ranked[which(new.ranked$HMS_ID %in% new.denom$HMS_ID),]

#change rankeddenom colnames to match denom_fordelivery
colnames(old.ranked)[which(colnames(old.ranked)=="Grp1")]<-"Grp1Rank"
colnames(old.ranked)[which(colnames(old.ranked)=="Grp2")]<-"Grp2Rank"
colnames(new.ranked)[which(colnames(new.ranked)=="Grp1")]<-"Grp1Rank"
colnames(new.ranked)[which(colnames(new.ranked)=="Grp2")]<-"Grp2Rank"
...
if(any(grepl("Grp.*Cnt",colnames(co.ranked)))){
...
if (! grp2test){
 ...
 if(any(grepl("Grp1Cnt.x", colnames(co.ranked)))){
  ...
  print.dx.10<-test.range(1.1,5,co.ranked,"NumConnEnt.x","NumConnEnt.y","Grp1 denom good 10% =")
  print.dx.25<-test.range(1.25,10,co.ranked,"NumConnEnt.x","NumConnEnt.y","Grp1 denom good 25% =")
 }
...
if(! is.ina){
 ...
 sink(paste(outdir,"scatterqc_output.txt",sep="/"),append=T)
}
...
write.table(bad,file=paste(outdir,"indiv_bad_records.txt",sep="/"),row.names=F,col.names=T,quote=F,sep="\t")
...
write.table(bad,file=paste(outdir,"org_bad_records.txt",sep="/"),row.names=F,col.names=T,quote=F,sep="\t")
...
write.table(bad,file=paste(outdir,"pxdx_bad_records.txt",sep="/"),row.names=F,col.names=T,quote=F,sep="\t")
...
plot.heatmap(co.indiv[,2],co.indiv[,3],"Indivs Volume Heat Map")
...
plot.heatmap(co.org[,2],co.org[,3],"Orgs Volume Heat Map")
...
###
plot.heatmap(co.pxdx[,3],co.pxdx[,4],"PxDx Volume Heat Map")
...
plot.binbars(indiv.binned,"Indivs Binned Changes")
plot.binbars(org.binned,"Orgs Binned Changes")
plot.binbars(pxdx.binned,"PxDx Binned Changes")
...
print(table(merged.indiv.rank[, 2:3],exclude=NULL))
...
print(table(merged.org.rank[, 2:3],exclude=NULL))
