...
##[3]LOAD IN TABLES FROM MILESTONES FOLDER.
  if(file.exists("HMS_Individuals.tab")) {
pxdx	  <-	read.table("HMS_PxDx.tab", header=T, sep="\t", quote="", colClasses=c("ZIP"="character", "ZIP4"="character","PHONE1"="character", "PHONE2"="character", "FAX1"="character","FAX2"="character"), comment.char="", as.is=T, na.strings="")
indivs  <-	read.table("HMS_Individuals.tab", header=T, sep="\t", quote="", colClasses=c("PRIMARY_ZIP"="character", "PRIMARY_PHONE1"="character", "PRIMARY_PHONE2"="character", "PRIMARY_FAX"="character"), comment.char="", as.is=T, na.strings="")
orgs	  <-	read.table("HMS_Organizations.tab", header=T, sep="\t", quote="", colClasses=c("ZIP"="character", "ZIP4"="character", "PHONE1"="character", "PHONE2"="character", "FAX1"="character","FAX2"="character"), comment.char="", as.is=T, na.strings="")
...
} else if(file.exists("individuals.tab")) {
...
#   col_fac			<- grep("FAC", colnames(orgs))
#   col_fac_natl_rank		<- intersect(col_natl_rank,col_fac)
#   col_fac_rank		<- intersect(col_fac, col_rank)
#   col_prac_fac_rank		<- intersect(col_fac_rank,col_prac)
