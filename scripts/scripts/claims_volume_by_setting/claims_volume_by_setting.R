#arg 1 should be a list of codes or a file listing codes, arg 2 is dx or px
...
if(grepl(".tab",fullcodes)){
 fullcodes<-read.table(fullcodes,quote="",sep="",header=F,stringsAsFactors=F,colClasses="character")
 fullcodes<-paste("'",paste(fullcodes[,1],collapse="','"),"'",sep="")
 #print(fullcodes)
}else{
 fullcodes=args[1]
}
...
#codes in aggr are adnl_code, so no periods
fullcodes<-gsub("[.]","",fullcodes)
