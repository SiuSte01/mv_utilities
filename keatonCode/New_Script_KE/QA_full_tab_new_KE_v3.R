...
#Added in August 2019 to avoid scientific notation results if there are small numbers in output.
...
for (c in col_fac_natl_rank) {
  ...
  matched <- which(compare[,2] != compare[,3])
  if(length(matched) > 0) {
    cat("\n\t\t***WARNING: CHECK FAILED! Facility National Rank is not matched for these POIDs:***\n")
    cat("\n")
    print(compare[matched,1])
    cat("\n")
 }
...
if(pxdx_indiv==100){
  ...
  cat("\n\t\t**WARNING: CHECK FAILED! At least one PIID in PxDx table is not in Individuals table.\n")
}
...
if(pxdx_org==100){
  ...
  cat("\n\t\t**WARNING: CHECK FAILED! At least one POID in PxDx table is not in Organizations table.\n")
}
  ...
  if(milestonescheck==FALSE & qacheck==FALSE){
    cat("\n\t\t**WARNING: CHECK FAILED! This script was not run from the QA or milestones folder.\n")
  }
  
  
  if(dir.exists("../Projections")==FALSE){
    cat("\t\t**WARNING: CHECK SKIPPED! No projections folder found!\n")
  }else if(qacheck == TRUE){
    #If running script from QA folder 
...
nrow(orgs)
  ...
  cat("Are buckets labeled properly with diagnosis/procedure bucket name in delivery tables?\n")
  
  cat("Does an unprojected version of this bucket exist? If so, confirm that row counts for Indivs, Orgs, and PxDx table line up with unprojected version.\n")
  ...
  cat("Check distribution of PIID Physician National Rank in Decile_Distribution_Graphs.pdf.\n")
  cat("\n")
  
  fixedranks <- (1:10)
  #convert rank table into data form
  # pdf("PXDX_TABLE_Check_dist_PIIDs_PRACTITIONER_NATL_RANK.pdf",onefile=T)
  pdf("Decile_Distribution_Graphs.pdf",onefile=T)
...
#   ex <- barplot(mer$Freq,main=sub("_PRACTITIONER_NATL_RANK", "", colnames(pxdx)[c]),names.arg=mer$x)
    ex <- barplot(mer$Freq,main=paste((colnames(pxdx)[c]),"\nPxDx Table",sep=""),names.arg=mer$x)
  ...
  #	perform a Quintile. If more than 5 numbers are missing and a trecile looks more promising discuss
  #	with Dilip/Sales and perform Trecile. 
  cat("\nCheck distribution of POID Facility National Rank in Decile_Distribution_Graphs.pdf.\n")
  cat("\n")
  ...
  cat("\nCheck distribution of PIID Physician National Rank in Decile_Distribution_Graphs.pdf.\n\n")
  ...
  for (c in col_prac_natl_rank) {
    ...
    ex <- barplot(mer$Freq,main=paste((colnames(indivs)[c]),"\nIndivs Table",sep=""),names.arg=mer$x)
    text(ex, mer$Freq+3, format(mer$Freq), xpd=TRUE, col="blue")
  }
  # null_device<-dev.off()
  ...
  for (c in col_fac_natl_rank) {
    ...
    ex <- barplot(mer$Freq,main=paste((colnames(orgs)[c]),"\nOrgs Table",sep=""),names.arg=mer$x)
    text(ex, mer$Freq+3, format(mer$Freq), xpd=TRUE, col="blue")
    cat("\n")
  }
