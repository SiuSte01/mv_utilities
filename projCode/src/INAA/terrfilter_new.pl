...
use strict;
use Data::Dumper;
use lib "/opt/local/marketview/conda/1/envs/master/lib/python3.7/site-packages/lib";
use MiscFunctions;
...
my $dbhA = DBI->connect('DBI:Oracle:PLDELDB','hms_pe','hms_pe123') or die "could not connect to database: ".DBI->errstr;
...
#MiscFunctions::screenPrintHash(hash=>\%wantidgrp1);
#exit 0;
...
#print "Teeheehee\n";
#MiscFunctions::screenPrintHash(hash=>\%grp1universe);
#exit 0;
...
if($ngr==2)
{
	...
	else
	{
		...
		if(scalar(keys %grp2universe) > 0)
		{
			...
			{
				if(defined $id2cnt{$id}	&& !defined $id2natrank{$id}{2})
				{
...
foreach my $l (@order)
{
	...
	next if($sormap{&linknatrank($l)} =~ m/^0/ || $sormap{&linksterrrank($l)}  =~ m/^0/);
	my ($p1,$p2)=split(":",$l);
	#DEBUG
	#print "|$l|\t|$p1|\t|$p2|\n";
...
#print "idsinnetworkkkkkkk\n";
#MiscFunctions::screenPrintHash(hash=>\%idsinnetwork);
#exit 0;
...
print OUT1 defined $grp1universe{$grp1filtkey}{Info} ? $grp1universe{$grp1filtkey}{Info} : $wantidgrp1{$grp1filtkey}{Info},"\t$grp1terrheader\n";
my %combinedidsgrp1;
#print "Broski, wut?\n";
#MiscFunctions::screenPrintHash(hash=>\%wantidgrp1);
#exit 0;
...
#print "GUHHHHHHH!!!\n";
#MiscFunctions::screenPrintHash(hash=>\%combinedidsgrp1);
#exit 0;
foreach my $id (keys %combinedidsgrp1)
{
	...
	if((defined $grp1universe{$id}{Info} || defined $wantidgrp1{$id}{Info}) && defined $id2terrrank{$id}{$k})
	{
		print OUT1 defined $grp1universe{$id}{Info} ? $grp1universe{$id}{Info} : $wantidgrp1{$id}{Info},"\t$id2terrrank{$id}{$k}\n";
	}
	#}
}
close(OUT1);
#print "this supposedly happens\n";
#exit 0;
...
if($ngr == 2)
{
	...
	$grp2universe{$grp2filtkey}{Info} : $wantidgrp2{$grp2filtkey}{Info},"\t$grp2terrheader\n";
	if($have0sormap eq "y")
	{
		foreach my $id (keys %idsinnetwork)
		{
	...
	foreach my $id (keys %combinedidsgrp2)
	{
		...
		if((defined $grp2universe{$id}{Info} || defined $wantidgrp2{$id}{Info}) && defined $id2terrrank{$id}{2})
		{
			print OUT2 defined $grp2universe{$id}{Info} ? $grp2universe{$id}{Info} : $wantidgrp2{$id}{Info},"\t$id2terrrank{$id}{2}\n";
		}
	}
	#}
}
...
{
	#print "ZOMGZOMGZOMG grp1affilsfile is: " . $grp1affilsfile . "\n";
	#print "this happens\n";
	#MiscFunctions::screenPrintHash(hash=>\%combinedidsgrp1);
	#exit 0;
...
if($grp1secondfile ne "")
{
	...
	print "number of Grp1 second file ids to include = ",scalar(keys %wantsecondkey),"\n";
...
sub getHCPaddr
{
	my $sql1=qq|select d.HMS_PIID,c.address_line1,c.address_line2,c.city_name,
...
sub getHCPphfx
{
	my $sql1=qq|select d.HMS_PIID,b.phone_number,b.rank
		from
			ddb_|.$vint.qq|.m_address c,
			ddb_|.$vint.qq|.m_practitioner_address d,
	...
	my $sql2=qq|select d.HMS_PIID,b.fax_number,b.rank
		from
			ddb_|.$vint.qq|.m_address c,
			ddb_|.$vint.qq|.m_practitioner_address d,
...
sub getcount
{
	...
	if($grp == 1)
	{
		if($grp1decilecol ne "")
		{
			$c=defined $grp1universe{$id}{Count} ? $grp1universe{$id}{Count} : $wantidgrp1{$id}{Count};
		}
	}
	elsif($grp == 2)
	{
		if($grp2decilecol ne "")
		{
			$c=defined $grp2universe{$id}{Count} ? $grp2universe{$id}{Count} : $wantidgrp2{$id}{Count};
		}
	}
	return $c;
}
...
sub getHCOinfo
{
	my $sql1=qq|select a.hms_poid,a.org_name, c.address_line1, c.address_line2, c.city_name,
			c.state_code, c.address_zip5, c.address_zip4, c.fips5_code,
...
sub getHCOphfx
{
	my $sql1=qq|select c.HMS_POID,b.org_phone_number,b.rank
		from
	...
	my $sql2=qq|select c.HMS_POID,b.org_fax_number,b.rank
		from
			ddb_|.$vint.qq|.m_organization_address c,
